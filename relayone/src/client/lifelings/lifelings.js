...
const LifeEngineFinalization = (typeof FinalizationRegistry === 'undefined')
    ? { register: () => {}, unregister: () => {} }
    : new FinalizationRegistry(ptr => wasm.__wbg_lifeengine_free(ptr >>> 0, 1));

export class LifeEngine {
    __destroy_into_raw() {
        const ptr = this.__wbg_ptr;
        this.__wbg_ptr = 0;
        LifeEngineFinalization.unregister(this);
        return ptr;
    }
    free() {
        const ptr = this.__destroy_into_raw();
        wasm.__wbg_lifeengine_free(ptr, 0);
    }
    /**
     ...
     * @param {boolean} is_blurry
     */
    constructor(canvas, canvas_width, canvas_height, cell_size, is_blurry) {
        const ret = wasm.lifeengine_new(canvas, canvas_width, canvas_height, cell_size, is_blurry);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
        this.__wbg_ptr = ret[0] >>> 0;
        LifeEngineFinalization.register(this, this.__wbg_ptr, this);
        return this;
    }
    tick() {
        wasm.lifeengine_tick(this.__wbg_ptr);
    }
    reset() {
        wasm.lifeengine_reset(this.__wbg_ptr);
    }
    /**
     * @param {number} width
     * @param {number} height
     */
    resize(width, height) {
        wasm.lifeengine_resize(this.__wbg_ptr, width, height);
    }
}
if (Symbol.dispose) LifeEngine.prototype[Symbol.dispose] = LifeEngine.prototype.free;
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('lifelings_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
